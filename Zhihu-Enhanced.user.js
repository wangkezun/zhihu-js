...
var menu_ALL = [
    ...
    [
      "menu_blockLowCount",
      "屏蔽低赞低评",
      "设置要屏蔽 低于多少赞同/评价 的回答/文章（默认不需要留空即可）<br/>（例如设置 0 则无人赞同/评价的回答/文章会被屏蔽<br/>（例如设置 20 则赞同/评价数量低于 20 的回答/文章会被屏蔽<br/>（修改后，后续加载的回答/文章会立即生效，但不影响当前网页已有内容",
      "",
    ],
    ...
    [
      "menu_blockTypeFollowAgree",
      "赞同了XX [关注页]",
      "赞同了XX（关注页）",
      false,
    ],
    [
      "menu_blockTypeFollowQuestion",
      "关注了XX [关注页]",
      "关注了XX（关注页）",
      false,
    ],
...
function registerMenuCommand() {
  ...
  for (let i = 0; i < menu_ALL.length; i++) {
    ...
    if (menu_ALL[i][0] === "menu_blockLowCount") {
      ...
      // 只有 [屏蔽指定关键词] 启用时，才注册菜单 [屏蔽关键词 - 评论区]
      ...
      // 只有 [屏蔽指定关键词] 启用时，才注册菜单 [添加选中文字到屏蔽词]
      if (menu_value("menu_blockKeywords"))
        menu_ID[i] = GM_registerMenuCommand(
          `#️⃣ ${menu_ALL[i][1]}`,
          function () {
            addSelectedKeywordToBlocklist();
          },
        );
      ...
      menu_ALL[i][0].indexOf("menu_blockType") == -1 &&
      menu_ALL[i][0] != "menu_blockYanXuan" &&
      menu_ALL[i][0].indexOf("menu_blockLow") == -1
    ) {
      // 排除使用单独设置界面的 屏蔽指定类别 项
      menu_ID[i] = GM_registerMenuCommand(
        `${menu_ALL[i][3] ? "✅" : "❌"} ${menu_ALL[i][1]}`,
        function () {
          menu_switch(
...
[data-theme="dark"] .zhihuE_SettingMain hr {border: 0.5px solid #2d333b;}</style>
  ...
  for (let i = 0; i < menu.length; i++) {
    if (menu[i][0].indexOf("menu_blockLow") === 0) {
      _html += `<label>${menu[i][1]}：<input name="${menu[i][0]}" type="text" oninput="value=value.replace(/[^\\d]/g,'')" value="${escapeHtml(GM_getValue(menu[i][0]))}" style="width: 50px;"></label>${_br}`;
    ...
    if (GM_getValue(menu)) {
      ...
      if (item_ContentItem && item_ContentItem.dataset.zaExtraModule) {
        ...
        if (
          ...
          console.log(
            "已屏蔽" +
              (type === "upvote_num" ? "低赞" : "低评") +
              (item_ContentItem.classList.contains("AnswerItem")
                ? "回答"
                : "文章") +
              "：",
            item2.card.content[type] + "<" + GM_getValue(menu),
            item,
          );
          item.hidden = true;
  ...
  function blockType_(titleA) {
    ...
    if (location.pathname === "/search") {
      ...
      if (type === "follow") {
        if (
          (menu_value("menu_blockTypeFollowAgree") &&
            titleA.className.indexOf("FeedSource-byline") != -1) ||
          (menu_value("menu_blockTypeFollowQuestion") &&
    ...
    for (const mutation of mutationsList) {
      for (const target of mutation.addedNodes) {
        ...
        if (
          ...
          target.href.indexOf("https://zhida.zhihu.com/search?") > -1
        ) {
    ...
    if (
      ...
      if (location.pathname.indexOf("waiting") == -1) {
        ...
        if (location.pathname.indexOf("answer") == -1) {
          ...
          // 将回答页的的查看全部回答选项去掉默认的点击事件改成静态链接，为了避免功能交叉混乱
      ...
      ["/", "/hot", "/follow", "/column-square", "/ring-feeds"].indexOf(
        location.pathname,
      ) !== -1
    ) {
      //    首页 //
